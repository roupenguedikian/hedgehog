#!/usr/bin/env python3
"""Aster Opportunity Scanner — find best cross-venue hedges for every Aster symbol.

Fetches live funding rates from all Aster products, then scans 9 other venues
to find the best hedge pair (short high-rate + long low-rate) where Aster is
always one leg.

Usage:
    python3 scripts/exchanges/aster_opportunities.py [--min-volume=100000] [--top=50]
    python3 scripts/exchanges/aster_opportunities.py --loop[=60]
"""
import asyncio
import json
import math
...
def load_ema(tag: str = "aster") -> tuple[float, dict]:
...
def save_ema(ts: float, state: dict, tag: str = "aster"):
...
# ═══════════════════════════════════════════════════════════════
# ASTER HEDGE PAIR BUILDER
# ═══════════════════════════════════════════════════════════════

def build_aster_pairs(aster_data, other_venue_data, min_vol, ema):
    """For each Aster symbol, find the best hedge on another venue.

    Aster is always one leg. For each symbol present on Aster AND at
    least one other venue (both above min_vol), pick the direction and
    counter-venue that maximises the composite score.
    """
    pairs = []
    for symbol, ast_info in aster_data.items():
        ast_apy = ast_info["apy"]
        ast_vol = ast_info["volume"]
        ...
        for venue, symbols in other_venue_data.items():
            ...
            if ast_apy >= other_apy:
                sv, sa, svol = "Aster", ast_apy, ast_vol
                lv, la, lvol = venue, other_apy, other_vol
            else:
                sv, sa, svol = venue, other_apy, other_vol
                lv, la, lvol = "Aster", ast_apy, ast_vol
            ...
            if score > best_score:
                ...
                best_pair = {
                    ...
                    "be_hours": be_hours,
                    "min_vol": min_leg_vol,
                    "ast_side": "SHORT" if sv == "Aster" else "LONG",
                    "counter_venue": lv if sv == "Aster" else sv,
                    "score": score,
                }
...
def display_pairs(pairs, top_n, ema_age_str, ast_total):
    """Print Aster-centric hedge pair opportunity table."""
    print(f"\n  EMA: tau=1h, {ema_age_str}")
    print(f"  Score = log-compressed EMA x persistence x volume_conf x venue_quality")
    print(f"  Aster symbols: {ast_total}  |  Hedgeable: {len(pairs)}")
    print(
        f"\n  {'#':>3} {'SCORE':>6} {'SYMBOL':<8} {'AST':>5} "
    ...
    for i, p in enumerate(pairs[:top_n], 1):
        ...
        print(
            f"  {i:>3} {score_str} {p['symbol']:<8} {ast_tag:>13} "
...
async def scan_once(min_vol: float, top_n: int):
    ...
    print("=" * 140)
    print(f"  ASTER OPPORTUNITIES — {ts_str}")
    print(f"  Best hedge for each Aster symbol across 9 other venues")
    print(f"  Volume filter: >${min_vol / 1000:.0f}k on counter-leg  |  1h EMA")
    print("=" * 140)
    print("  Fetching Aster rates + 9 counter-venues...", flush=True)
    ...
    async with httpx.AsyncClient(timeout=25.0) as client:
        # Fetch Aster + all other venues in parallel
        all_tasks = [fetch_aster(client)] + [fn(client) for _, fn in other_venues]
        results = await asyncio.gather(*all_tasks, return_exceptions=True)

    # Unpack Aster result
    ast_result = results[0]
    if isinstance(ast_result, Exception):
        print(f"  !! Aster: {ast_result}")
        print(f"  Cannot scan without Aster data.\n")
        return
    ast_data = ast_result
    ast_qualified = sum(1 for v in ast_data.values() if v.get("volume", 0) >= min_vol)
    print(f"  * Aster: {len(ast_data)} symbols, {ast_qualified} above ${min_vol / 1000:.0f}k vol")
    ...
    # Build current APY map for EMA (Aster + all venues with volume)
    current_apy: dict[str, float] = {}
    for symbol, info in ast_data.items():
        current_apy[f"Aster:{symbol}"] = info["apy"]
    ...
    # Build Aster-centric hedge pairs
    pairs = build_aster_pairs(ast_data, other_data, min_vol, ema)
    ...
    display_pairs(pairs, top_n, ema_age, len(ast_data))

    # Summary
    print(f"\n  {len(pairs)} Aster hedge pairs found (counter-leg >${min_vol / 1000:.0f}k vol)")
    if pairs:
        ...
        # Aster side breakdown
        ast_short = sum(1 for p in pairs if p["ast_side"] == "SHORT")
        ast_long = sum(1 for p in pairs if p["ast_side"] == "LONG")
        print(f"  Aster side: {R}{ast_short} short{X}  |  {G}{ast_long} long{X}")
