...
Usage: python3 scripts/drift_query.py [markets|funding|account|positions|orders|fills|income|all] [address]
"""
import asyncio
import os
import sys
...
# ═══════════════════════════════════════════════════════════════
# MARKETS — top perps by volume with funding rates
# ═══════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════
# FUNDING — average rates + recent history
# ═══════════════════════════════════════════════════════════════
...
async def query_positions(client: httpx.AsyncClient, address: str):
    ...
    try:
        ...
        print(f"\n  {'SYMBOL':<14} {'SIDE':>5} {'SIZE':>12} {'NOTIONAL':>12} "
              f"{'ENTRY':>12} {'MARK':>12} {'uPnL':>10} {'LIQ':>12}")
        print("  " + "-" * 95)
        for p in positions:
...
async def query_orders(client: httpx.AsyncClient, address: str):
    ...
    try:
        ...
        print(f"\n  {'SYMBOL':<14} {'SIDE':>5} {'TYPE':>8} {'PRICE':>12} {'SIZE':>12}")
        print("  " + "-" * 60)
        for o in orders:
            ...
            print(f"  {symbol:<14} {direction:>5} {order_type:>8} ${price:>11,.4f} {size:>12.4f}")
...
async def query_fills(client: httpx.AsyncClient, address: str):
    ...
    try:
        ...
        for rec in records:
            ...
            role = "T" if is_taker else "M"
            print(f"  {time_str:<20} {symbol:<12} {str(side):<6} {base_filled:>12.4f} "
                  f"${quote_filled:>11,.2f} ${price:>11,.4f} ${fee:>9,.4f} {role}")
...
PUBLIC_SECTIONS = {"markets": query_markets, "funding": query_funding}
ACCT_SECTIONS = {
