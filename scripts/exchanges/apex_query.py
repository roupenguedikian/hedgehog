    ...
    for sym in symbols:
        try:
            ...
            if items and isinstance(items, list) and items[0]:
                ...
                oi_usd = oi * last
                # Check for 24h volume/turnover fields
                vol_raw = d.get("turnover24h") or d.get("volume24h") or None
                vol_usd = float(vol_raw) if vol_raw else 0.0
                assets.append({
                    "symbol": sym, "rate": rate, "predicted": pred,
                    "annualized": ann, "last": last, "hi": hi, "lo": lo,
                    "oi": oi, "oi_usd": oi_usd, "chg": chg, "index": idx,
                    "next_funding": next_t, "vol_usd": vol_usd,
                })
        except Exception:
            pass

    # Sort by 24h volume if available, otherwise by OI
    has_volume = any(a["vol_usd"] > 0 for a in assets)
    if has_volume:
        sorted_assets = sorted(assets, key=lambda a: a["vol_usd"], reverse=True)
    else:
        sorted_assets = sorted(assets, key=lambda a: a["oi_usd"], reverse=True)

    if has_volume:
        print(f"\n  {'SYMBOL':<12} {'RATE/8H':>12} {'ANNUAL':>8} {'PREDICTED':>12} "
              f"{'MARK/LAST':>12} {'24H CHG':>8} {'24H VOL':>14} {'OI (USD)':>14}")
        print("  " + "-" * 99)
        for a in sorted_assets:
            print(f"  {a['symbol']:<12} {a['rate']*100:>11.6f}% {a['annualized']*100:>+7.2f}% "
                  f"{a['predicted']*100:>11.6f}% ${a['last']:>11,.2f} "
                  f"{a['chg']*100:>+7.2f}% ${a['vol_usd']:>13,.0f} ${a['oi_usd']:>13,.0f}")
    else:
        print(f"\n  {'SYMBOL':<12} {'RATE/8H':>12} {'ANNUAL':>8} {'PREDICTED':>12} "
              f"{'MARK/LAST':>12} {'24H CHG':>8} {'OI (USD)':>14}")
        print("  " + "-" * 85)
        print("  (Note: 24h volume not available from ticker endpoint; sorted by OI)")
        for a in sorted_assets:
            print(f"  {a['symbol']:<12} {a['rate']*100:>11.6f}% {a['annualized']*100:>+7.2f}% "
                  f"{a['predicted']*100:>11.6f}% ${a['last']:>11,.2f} "
...
async def account(client):
    ...
    if bal and isinstance(bal, dict) and j2.get("code") is None:
        ...
        # Margin utilization %
        equity_raw = bal.get("totalEquityValue")
        im_raw = bal.get("initialMargin")
        if equity_raw and im_raw:
            try:
                equity_f = float(equity_raw)
                im_f = float(im_raw)
                if equity_f > 0:
                    margin_util = im_f / equity_f * 100
                    print(f"  {'Margin Utilization':<30s} {margin_util:>15.2f}%")
        ...
        if isinstance(bal, dict):
            ...
            if isinstance(pos_list, list) and pos_list:
                print(f"\n  {'SYMBOL':<14} {'SIDE':<6} {'SIZE':>10} {'ENTRY':>12} "
                      f"{'NOTIONAL':>12} {'MARK':>12} {'LEV':>6} {'uPnL':>12} {'LIQ':>12}")
                print("  " + "-" * 108)
                total_upnl = 0
                for p in pos_list:
                    sym = p.get("symbol", "?")
                    ...
                    # Compute notional if size and entry are available
                    try:
                        notional_val = abs(float(size)) * float(entry)
                        notional_str = f"${notional_val:>11,.2f}"
                    except (ValueError, TypeError):
                        notional_str = f"{'N/A':>12}"
                    mark_str = f"${float(mark):>11,.2f}" if mark else f"{'N/A':>12}"
                    lev_str = f"{leverage:>6}" if leverage else f"{'N/A':>6}"
                    total_upnl += upnl
                    print(f"  {sym:<14} {side:<6} {size:>10} ${float(entry):>11,.2f} "
                          f"{notional_str} {mark_str} {lev_str} "
                          f"${upnl:>+11,.2f} {liq:>12}")
...
async def orders(client):
    ...
    print(f"\n  {'SYMBOL':<14} {'SIDE':<6} {'TYPE':<10} {'PRICE':>12} "
          f"{'SIZE':>10} {'FILLED':>10} {'TIF':<6} {'STATUS':<12}")
    print("  " + "-" * 86)
    for o in order_list:
        sym = o.get("symbol", "?")
        side = o.get("side", "?")
...
async def fills(client):
    ...
    if isinstance(fill_list, list):
        print(f"\n  {'TIME':>20} {'SYMBOL':<14} {'SIDE':<6} {'TYPE':<8} "
              f"{'PRICE':>12} {'SIZE':>10} {'QUOTE VAL':>12} {'FEE':>10} {'M/T':<4}")
        print("  " + "-" * 102)
        for f in fill_list:
            sym = f.get("symbol", "?")
            side = f.get("side", "?")
            ...
            # Map direction to M(aker)/T(aker) label
            dir_map = {"MAKER": "M", "TAKER": "T", "maker": "M", "taker": "T"}
            mt_label = dir_map.get(direction, direction[:1] if direction and direction != "?" else "?")
            otype = f.get("orderType", "?")
            created = ts(f.get("createdAt", 0))
            # Quote value = price * size
            try:
                quote_val = float(price) * float(size)
                quote_str = f"${quote_val:>11,.2f}"
            except (ValueError, TypeError):
                quote_str = f"{'N/A':>12}"
            try:
            ...
            print(f"  {created:>20} {sym:<14} {side:<6} {otype:<8} "
                  f"${float(price):>11,.2f} {size:>10} {quote_str} {fee_str} {mt_label:<4}")
