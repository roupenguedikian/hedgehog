---
name: aster-limit
description: Place a limit order on Aster DEX. Use when the user wants to buy, sell, long, short, or place an order on Aster.
allowed-tools: Bash
argument-hint: "<symbol> <long|short> <size> <price> [--tif GTC|IOC|GTX] [--reduce-only]"
---

# Aster DEX Limit Order

Place a limit order on Aster DEX (BNB Chain) via the Binance-compatible REST API. Requires `ASTER_API_KEY` and `ASTER_API_SECRET` in `.env`.
...
```bash
.venv/bin/python scripts/aster_order.py <symbol> <side> <size> <price> [--tif GTC] [--reduce-only] --dry-run
```

### Execute (only after user confirms):

```bash
.venv/bin/python scripts/aster_order.py <symbol> <side> <size> <price> [--tif GTC] [--reduce-only]
```
...
.venv/bin/python scripts/aster_order.py BTC long 0.001 70000 --dry-run

# Execute: limit sell 10 SOL at $95
.venv/bin/python scripts/aster_order.py SOL short 10 95

# Post-only buy 100 DOGE at $0.10
.venv/bin/python scripts/aster_order.py DOGE long 100 0.10 --tif GTX

# Close a position (reduce only)
.venv/bin/python scripts/aster_order.py ETH short 0.5 2200 --reduce-only
```
...
- Market context: mark price, index price, funding rate, best bid/ask, spread, how far the limit is from mid
- Account: NAV, free margin, whether the order is affordable
- Ask for explicit confirmation

After execution, show:
- Order result: status, order ID, fill price if any
