#!/usr/bin/env python3
"""Ethereal Opportunity Scanner — find best cross-venue hedges for every Ethereal symbol.

Fetches live funding rates from all Ethereal products, then scans 9 other venues
to find the best hedge pair (short high-rate + long low-rate) where Ethereal is
always one leg.

Usage:
    python3 scripts/exchanges/ethereal_opportunities.py [--min-volume=100000] [--top=50]
    python3 scripts/exchanges/ethereal_opportunities.py --loop[=60]
"""
import asyncio
import json
import math
...
async def fetch_extended(client: httpx.AsyncClient) -> dict:
    """Extended (Starknet): 1h funding cycle."""
    base = "https://api.extended.exchange"
    try:
        resp = await client.get(f"{base}/v1/public/markets")
        resp.raise_for_status()
        markets = resp.json().get("markets", [])
    except Exception:
        return {}
    ...
    result = {}
    for m in markets:
        sym = m.get("baseAsset", "").upper()
        if not sym:
            continue
        rate = float(m.get("fundingRate") or 0)
        volume = float(m.get("volume24h") or 0)
        apy = rate * 8760 * 100  # 1h cycle
        result[sym] = {"apy": apy, "volume": volume}
    return result
...
def load_ema(tag: str = "ethereal") -> tuple[float, dict]:
...
def save_ema(ts: float, state: dict, tag: str = "ethereal"):
...
# ═══════════════════════════════════════════════════════════════
# ETHEREAL HEDGE PAIR BUILDER
# ═══════════════════════════════════════════════════════════════

def build_ethereal_pairs(ethereal_data, other_venue_data, min_vol, ema):
    """For each Ethereal symbol, find the best hedge on another venue.

    Ethereal is always one leg. For each symbol present on Ethereal AND at
    least one other venue (both above min_vol), pick the direction and
    counter-venue that maximises the composite score.
    """
    pairs = []
    for symbol, eth_info in ethereal_data.items():
        eth_apy = eth_info["apy"]
        eth_vol = eth_info["volume"]

        best_pair = None
        best_score = -1

        for venue, symbols in other_venue_data.items():
            if symbol not in symbols:
                continue
            other = symbols[symbol]
            if other["volume"] < min_vol:
                continue

            other_apy = other["apy"]
            other_vol = other["volume"]

            # Orient: short = higher APY, long = lower APY
            if eth_apy >= other_apy:
                sv, sa, svol = "Ethereal", eth_apy, eth_vol
                lv, la, lvol = venue, other_apy, other_vol
            else:
                sv, sa, svol = venue, other_apy, other_vol
                lv, la, lvol = "Ethereal", eth_apy, eth_vol
            ...
            if score > best_score:
                ...
                best_pair = {
                    ...
                    "be_hours": be_hours,
                    "min_vol": min_leg_vol,
                    "eth_side": "SHORT" if sv == "Ethereal" else "LONG",
                    "counter_venue": lv if sv == "Ethereal" else sv,
                    "score": score,
                }
...
def display_pairs(pairs, top_n, ema_age_str, eth_total):
    """Print Ethereal-centric hedge pair opportunity table."""
    print(f"\n  EMA: tau=1h, {ema_age_str}")
    print(f"  Score = log-compressed EMA x persistence x volume_conf x venue_quality")
    print(f"  Ethereal symbols: {eth_total}  |  Hedgeable: {len(pairs)}")
    print(
        f"\n  {'#':>3} {'SCORE':>6} {'SYMBOL':<8} {'ETH':>5} "
    ...
    for i, p in enumerate(pairs[:top_n], 1):
        ...
        print(
            f"  {i:>3} {score_str} {p['symbol']:<8} {eth_tag:>13} "
...
async def scan_once(min_vol: float, top_n: int):
    ...
    print("=" * 140)
    print(f"  ETHEREAL OPPORTUNITIES — {ts_str}")
    print(f"  Best hedge for each Ethereal symbol across 9 other venues")
    print(f"  Volume filter: >${min_vol / 1000:.0f}k on counter-leg  |  1h EMA")
    print("=" * 140)
    print("  Fetching Ethereal rates + 9 counter-venues...", flush=True)
    ...
    async with httpx.AsyncClient(timeout=25.0) as client:
        # Fetch Ethereal + all other venues in parallel
        all_tasks = [fetch_ethereal(client)] + [fn(client) for _, fn in other_venues]
        results = await asyncio.gather(*all_tasks, return_exceptions=True)

    # Unpack Ethereal result
    eth_result = results[0]
    if isinstance(eth_result, Exception):
        print(f"  !! Ethereal: {eth_result}")
        print(f"  Cannot scan without Ethereal data.\n")
        return
    eth_data = eth_result
    eth_qualified = sum(1 for v in eth_data.values() if v.get("volume", 0) >= min_vol)
    print(f"  * Ethereal: {len(eth_data)} symbols, {eth_qualified} above ${min_vol / 1000:.0f}k vol")

    # Collect other venue data
    other_data: dict[str, dict] = {}
    for (name, _), result in zip(other_venues, results[1:]):
        if isinstance(result, Exception):
            print(f"  ! {name}: {result}")
            other_data[name] = {}
        else:
            status = result.pop("__status__", None) if isinstance(result, dict) else None
            other_data[name] = result
            if status == "blocked":
    ...
    # Build current APY map for EMA (Ethereal + all venues with volume)
    current_apy: dict[str, float] = {}
    for symbol, info in eth_data.items():
        current_apy[f"Ethereal:{symbol}"] = info["apy"]
    for venue, symbols in other_data.items():
    ...
    # Build Ethereal-centric hedge pairs
    pairs = build_ethereal_pairs(eth_data, other_data, min_vol, ema)
    ...
    display_pairs(pairs, top_n, ema_age, len(eth_data))

    # Summary
    print(f"\n  {len(pairs)} Ethereal hedge pairs found (counter-leg >${min_vol / 1000:.0f}k vol)")
    if pairs:
        ...
        # Ethereal side breakdown
        eth_short = sum(1 for p in pairs if p["eth_side"] == "SHORT")
        eth_long = sum(1 for p in pairs if p["eth_side"] == "LONG")
        print(f"  Ethereal side: {R}{eth_short} short{X}  |  {G}{eth_long} long{X}")

        # Top counter-venues
        counter_counts: dict[str, int] = defaultdict(int)
        for p in pairs:
            counter_counts[p["counter_venue"]] += 1
        top_counters = sorted(counter_counts.items(), key=lambda x: x[1], reverse=True)
        parts = [f"{name}({cnt})" for name, cnt in top_counters[:5]]
        print(f"  Top counter-venues: {', '.join(parts)}")
    ...
    for arg in sys.argv[1:]:
        if arg.startswith("--min-volume="):
            ...
            if "=" in arg:
                ...
                loop_interval = 60  # default loop interval
