---
name: hl-limit
description: Place a limit order on Hyperliquid. Use when the user wants to buy, sell, long, short, or place an order on Hyperliquid.
allowed-tools: Bash
...
Place a limit order on Hyperliquid via the SDK. Requires `HYPERLIQUID_PRIVATE_KEY` in `.env`.
...
- **--tif** — time-in-force (optional, default GTC):
  ...
  - `ALO` — Add Liquidity Only (maker only, rejected if would cross)
...
```bash
.venv/bin/python scripts/hl_order.py <symbol> <side> <size> <price> [--tif GTC] [--reduce-only] --dry-run
```

### Execute (only after user confirms):

```bash
.venv/bin/python scripts/hl_order.py <symbol> <side> <size> <price> [--tif GTC] [--reduce-only]
```
...
.venv/bin/python scripts/hl_order.py BTC long 0.001 70000 --dry-run

# Execute: limit sell 10 SOL at $95
.venv/bin/python scripts/hl_order.py SOL short 10 95

# Maker-only buy 100 DOGE at $0.10
.venv/bin/python scripts/hl_order.py DOGE long 100 0.10 --tif ALO

# Close a position (reduce only)
.venv/bin/python scripts/hl_order.py ETH short 0.5 2200 --reduce-only
```
...
- Order result: filled/resting, order ID, fill price if any
